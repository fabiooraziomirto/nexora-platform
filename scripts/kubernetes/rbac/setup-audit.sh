...
if kubectl get deployment fluentd -n nxr-monitoring &> /dev/null || \
   kubectl get daemonset fluent-bit -n nxr-monitoring &> /dev/null; then
...
echo "  kubectl logs -n nxr -l app=rbac-service | grep audit"
