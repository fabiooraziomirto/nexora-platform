...
async def _publish_event(action: str, resource_id: str, payload: dict[str, Any]) -> None:
    if not producer:
        ...
        return
    event_type = f"dns-record.{action}"
...
async def observability_middleware(request: Request, call_next):
    ...
    HTTP_REQUESTS_TOTAL.labels("dns-service", request.method, request.url.path, str(response.status_code)).inc()
    HTTP_REQUEST_DURATION_SECONDS.labels("dns-service", request.method, request.url.path).observe(elapsed)
    logger.info(json.dumps({"service": "dns-service", "trace_id": trace_id, "correlation_id": correlation_id, "method": request.method, "path": request.url.path, "status": response.status_code, "duration_s": round(elapsed, 6)}))
    return response
...
async def create_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    await _publish_event("created", record.id, response)
    return response
...
async def delete_record(record_id: str) -> None:
    with SessionLocal() as db:
        ...
        db.delete(record)
        db.commit()
    await _publish_event("deleted", record_id, deleted_payload)
