...
from dns_service.core.config import (
...
from dns_service.core.database import Base, engine, SessionLocal
from dns_service.core.metrics import HTTP_REQUEST_DURATION_SECONDS, HTTP_REQUESTS_TOTAL
import dns_service.core.events as _events
from dns_service.models.dns_record import DNSRecord
...
async def observability_middleware(request: Request, call_next):
    ...
    HTTP_REQUESTS_TOTAL.labels(
        "dns-service", request.method, request.url.path, str(response.status_code)
    ).inc()
    HTTP_REQUEST_DURATION_SECONDS.labels(
        "dns-service", request.method, request.url.path
    ).observe(elapsed)
