...
export default function Login() {
  ...
  const devAuthEnabled = !configured && (import.meta.env.VITE_DEV_AUTH_ENABLED ?? 'true') !== 'false'
      ...
      <div className="w-full max-w-sm bg-white border border-slate-200 rounded p-6 shadow-sm">
        ...
        {!configured && (
          <div className="mb-4 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            Keycloak is not configured for this UI build. Development access is available.
          </div>
        )}
        <button
          onClick={() => auth.login()}
          disabled={!configured && !devAuthEnabled}
        ...
        >
          {configured ? 'Sign in with Keycloak' : 'Continue in development mode'}
        </button>
      </div>
    </div>
  )
